...
from Conv1x1BottleNeck.MyNet import MyNet
...
class Trainer:
    ...
    def train(self):
        if os.path.exists("../models/conv1x1bottleneck.pth"):
            self.net = torch.load("../models/conv1x1bottleneck.pth")
            print("exists")
        BATCH_SIZE = 76
        NUM_EPOCHS = 10
        ...
        for epochs in range(NUM_EPOCHS):
            ...
            torch.save(self.net, "../models/conv1x1bottleneck.pth")

    def test(self):
        testloader = data.DataLoader(dataset=self.test_dataset, batch_size=50, shuffle=True)
        net = torch.load("../models/conv1x1bottleneck.pth")
        net.eval()  # 用了BN 测试时加上eval函数
        total = 0
        for epochs,(x, y) in enumerate(testloader):
            ...
            for j, i in enumerate(axes[index]):  # (坐标还原)
                ...
                plt.pause(1)
                """
                batch_size=50还会内存溢出，原因是没有清空变量j,i
                python有垃圾系统回收机制，变量在被引用时不会被回收，必须人为回收变量
                即使批次给小，还会内存溢出，因为变量一直在累加，数据i很大
                """
                del boxes, j, i, target_box, img, draw
